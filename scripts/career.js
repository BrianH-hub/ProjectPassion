...
firebase.database().ref('/companies/').orderByChild("startDate").once('value', function (snapshot) {
    ...
    snapshot.forEach(function (childSnapshot) {
        ...
        document.getElementById("dbStart").innerHTML =
            '<div class="company"><div class="d-flex"><img class="logo border border-dark rounded float-left" src="./logos/'+companyName+'.png"><div class="description"><p class="position">' + jobPosition + '</p><h4 class="">' + companyName + '</h4></div><div class="coDetails ml-auto"><h5>' + startDate + ' - ' + endDate + '</h5><h5>' + city + '</h5></div></div><div class="jobdetails pl-sm-5">' + jobDetails + '</div></div>'
